...
export default function RoomListing() {
  ...
  const [applied, setApplied] = useState({
    maxPrice:50000, minBeds:0, amenities:[], gender:"Any",
  });
  ...
  const filtered = rooms
    .filter(r => {
      if (searchCity && !r.city?.toLowerCase().includes(searchCity.toLowerCase())) return false;
      if (category !== "All" && r.category !== category) return false;
      if (r.price > applied.maxPrice) return false;
      if (applied.minBeds > 0 && r.availableBeds < applied.minBeds) return false;
      if (applied.gender !== "Any" && r.gender && r.gender !== applied.gender) return false;
      if (applied.amenities.length > 0 && !applied.amenities.every(a => r.amenities?.includes(a))) return false;
      return true;
    })
    .sort((a, b) => {
      ...
      if (sort === "beds")       return (b.availableBeds ?? 0) - (a.availableBeds ?? 0);
      return 0;
    });
  ...
  // Clear all filters
  const clearFilters = () => {
    setMaxPrice(50000); setMinBeds(0); setAmenities([]); setGender("Any");
  ...
  return (
    <Layout>
      <div style={{ background:"#0E0E0F", minHeight:"100vh" }}>
        ...
        {/* ── CATEGORY PILLS ── */}
        <div style={{ borderBottom:"1px solid rgba(201,151,58,0.12)", background:"#0E0E0F" }}>
          <div style={{ display:"flex", gap:0, overflowX:"auto", scrollbarWidth:"none", padding:"0 20px" }}>
            {CATEGORIES.map(c => (
              <button key={c.id} className={`rl-cat${category===c.id?" on":""}`}
        ...
        {/* ── TOP BAR ── */}
        <div style={{ padding:"14px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10, borderBottom:"1px solid rgba(201,151,58,0.08)" }}>

          {/* Left: search + filter btn */}
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>

            {/* Inline city search */}
            <div style={{ display:"flex", alignItems:"center", gap:7, background:"#1E1E21", border:"1px solid rgba(201,151,58,0.2)", borderRadius:10, padding:"7px 13px", transition:"border-color 0.2s" }}
              onFocus={e => e.currentTarget.style.borderColor="rgba(201,151,58,0.5)"}
              onBlur={e  => e.currentTarget.style.borderColor="rgba(201,151,58,0.2)"}>
              <MapPin size={13} style={{ color:"#C9973A", flexShrink:0 }}/>
              <input
                placeholder="Filter by city..."
                value={searchCity}
                onChange={e => setSearchCity(e.target.value)}
                style={{ background:"transparent", border:"none", outline:"none", fontSize:13, color:"#F2EDE6", width:140 }}/>
              {searchCity && (
                <button onClick={() => setSearchCity("")}
                  style={{ background:"none", border:"none", cursor:"pointer", color:"#5C5448", display:"flex" }}>
                  <X size={12}/>
                </button>
              )}
            </div>
            ...
            {/* Filters button */}
            <button className={`rl-filt-btn${activeCount>0?" active":""}`}
              onClick={() => setShowFilter(true)}>
              <SlidersHorizontal size={13}/>
              Filters
              {activeCount > 0 && <span className="rl-badge">{activeCount}</span>}
            </button>
            ...
            <div style={{ position:"relative" }}>
              ...
              <ChevronDown size={12} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", color:"#5C5448", pointerEvents:"none" }}/>
            </div>
          </div>
        ...
        <div style={{ padding:"20px", maxWidth:1400, margin:"0 auto" }}>
          {loading ? (
            ...
            <div style={{ textAlign:"center", padding:"80px 20px", animation:"rlFadeUp 0.5s ease both" }}>
              <div style={{ fontSize:56, marginBottom:16 }}>🔍</div>
              <h3 style={{ fontFamily:"Georgia,serif", fontSize:22, color:"#F2EDE6", marginBottom:8, fontWeight:400 }}>
                No rooms found
              </h3>
              ...
              <button onClick={clearFilters}
                style={{ padding:"10px 24px", background:"#C9973A", color:"#0E0E0F", border:"none", borderRadius:12, fontSize:13, fontWeight:600, cursor:"pointer" }}>
                Clear all filters
              </button>
            </div>
          ) : (
        ...
        {showFilter && (
          <>
            ...
            <div className="rl-drawer">
              {/* Drawer header */}
              <div style={{ padding:"18px 20px", borderBottom:"1px solid rgba(201,151,58,0.12)", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
                <div>
                  <h2 style={{ fontFamily:"Georgia,serif", fontSize:18, color:"#F2EDE6", fontWeight:400, marginBottom:2 }}>
                    Filters
                  </h2>
                  {activeCount > 0 && (
                    <p style={{ fontSize:11, color:"#C9973A" }}>{activeCount} active</p>
                  )}
                </div>
                <button onClick={() => setShowFilter(false)}
                  style={{ width:32, height:32, borderRadius:"50%", background:"rgba(201,151,58,0.08)", border:"1px solid rgba(201,151,58,0.2)", color:"#A09480", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
                  <X size={15}/>
                </button>
              </div>

              {/* Drawer body */}
              <div ref={drawerBodyRef} style={{ flex:1, overflowY:"auto", padding:"20px" }}>

                {/* ── Max price ── */}
                <div style={{ marginBottom:24 }}>
                  <span className="rl-section-lbl">Max price / month</span>
                  <div style={{ background:"#1E1E21", border:"1px solid rgba(201,151,58,0.15)", borderRadius:12, padding:"14px 16px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                      <span style={{ fontSize:11, color:"#5C5448" }}>₹1,000</span>
                      <span style={{ fontFamily:"Georgia,serif", fontSize:18, color:"#C9973A" }}>
                        ₹{Number(maxPrice).toLocaleString("en-IN")}
                      </span>
                    </div>
                    <input type="range" className="rl-range"
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    {["Any",1,2,3,"4+"].map(n => (
                      <button key={n}
                        ...
                        onClick={() => setMinBeds(n==="Any"?0:n==="4+"?4:Number(n))}>
                        {n}
                      </button>
                    ))}
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
                    {AMENITY_OPTIONS.map(a => (
                      <button key={a.id}
                        ...
                        <span style={{ color: amenities.includes(a.id) ? "#C9973A" : "#5C5448" }}>
                          {a.icon}
                        </span>
                        {a.label}
                        {amenities.includes(a.id) && (
                          <span style={{ marginLeft:"auto", fontSize:10, color:"#C9973A" }}>✓</span>
                        )}
                      </button>
                    ))}
                  </div>
              ...
              {/* Drawer footer */}
              <div style={{ padding:"16px 20px", borderTop:"1px solid rgba(201,151,58,0.12)", flexShrink:0 }}>
                <button className="rl-apply" onClick={applyFilters}>
                  Show {rooms.filter(r => {
                    if (searchCity && !r.city?.toLowerCase().includes(searchCity.toLowerCase())) return false;
                    if (category !== "All" && r.category !== category) return false;
                    if (r.price > maxPrice) return false;
                    if (Number(minBeds) > 0 && r.availableBeds < Number(minBeds)) return false;
                    if (gender !== "Any" && r.gender && r.gender !== gender) return false;
                    if (amenities.length > 0 && !amenities.every(a => r.amenities?.includes(a))) return false;
                    return true;
                  }).length} rooms
                </button>
                <button className="rl-clear" onClick={clearFilters}>Clear all filters</button>
              </div>
            </div>
          </>
        )}
